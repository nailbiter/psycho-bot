...
ORGANIZATION: 
     VERSION: ---
     CREATED: 2022-05-24T21:46:49.297765
    REVISION: ---
...
def download_df_from_google_sheets(creds, spreadsheet_id, sheet_name=None, initial_row=0):
    service = build('sheets', 'v4', credentials=creds)

    # Call the Sheets API
    sheet = service.spreadsheets()
    range_ = f"A{initial_row+1}:AZ"
...
def get_files_in_gdrive_folder(creds, folder_id, fields=["id", "name"]):
    drive_service = build('drive', 'v3', credentials=creds, )
    ...
    while True:
        ...
        for file in response.get('files', []):
            ...
            files_df.append({k: file.get(k) for k in fields})
